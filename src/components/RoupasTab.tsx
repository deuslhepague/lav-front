...
export default function RoupasTab({ userId, showToast, isOwner = true }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const d = await api.colecao(userId, {
        pagina, ordem,
        ...(cat   && { categoria: cat }),
        ...(rar   && { raridade: rar }),
        ...(busca && { busca }),
  ...
  }, [userId, pagina, cat, rar, busca, ordem])
  ...
  const grupos = roupas.reduce((acc, r) => {
    const g = r.categoria || 'outros'
    if (!acc[g]) acc[g] = { emoji: r.categoria_emoji || '📦', roupas: [] }
    acc[g].roupas.push(r)
    return acc
  }, {} as Record<string, { emoji: string; roupas: Roupa[] }>)

  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Filters */}
      <div className="flex-shrink-0 bg-bg px-4 pt-3 pb-0">
        {/* Search */}
        <div className="flex items-center gap-2 bg-s0 border border-border rounded-ios px-3 py-2 mb-3">
          ...
          />
          {buscaInput && (
            <button onClick={() => setBuscaInput('')} className="text-muted text-lg leading-none">×</button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="pb-20">
            {Object.entries(grupos).map(([catNome, g]) => (
              <div key={catNome}>
                {/* Group header */}
                <div className="sticky top-0 z-10 bg-bg/90 backdrop-blur-sm
            ...
            {totalPag > 1 && (
              <div className="flex items-center justify-center gap-4 py-4">
                <button
                  ...
                  className="text-acc text-sm font-medium disabled:text-muted disabled:cursor-default"
                >
                  ← Anterior
                </button>
      ...
      className={`flex-shrink-0 text-xs font-medium px-3 py-1.5 rounded-full
                  border transition-all duration-150 ${
                    active
                      ...
                      : 'bg-transparent text-sub border-border'
                  }`}
    >
      {label}
    </button>
